...
# 将人和帽子进行匹配
def match_people_with_hats(people_boxes, hat_boxes, iou_threshold=0):
    num_people = len(people_boxes)
    num_hats = len(hat_boxes)

    # 构建IoU矩阵
    cost_matrix = np.zeros((num_people, num_hats))
    for i, person_box in enumerate(people_boxes):
        for j, hat_box in enumerate(hat_boxes):
    ...
    for i, j in matches:
        match_dict[i] = j
    return match_dict, cost_matrix

# def revise_pred(workers):
#     result_tensor = torch.zeros(1, 6)
#     re_pred_list = []
#     for woker in workers:
#         re_pred_list.append()

class Worker:
    def __init__(self, xyxy, depth = -1):
        self.xyxy = xyxy
        self.depth = -1
        self.hat = None
        self.vest = None
    ...
    def assign_hat(self, hat):
        self.hat = hat

    def assign_vest(self, vest):
        self.vest = vest

    def relabel(self): # 根据match结果，重新设置label
        if self.hat and self.vest:
            self.re_label = 3 # WHV
        elif self.hat:
            self.re_label = 2 # WH
        elif self.vest:
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(re_pred):  # per image
            seen += 1
